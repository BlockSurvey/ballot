...
import { BLOCKSTACK_DEFAULT_GAIA_HUB_URL, DoesNotExist, getGlobalObject, InvalidStateError, megabytesToBytes, PayloadTooLargeError, SignatureVerificationError, } from '@stacks/common';
import { eciesGetJsonStringLength, getPublicKeyFromPrivate, publicKeyToAddress, signECDSA, verifyECDSA, } from '@stacks/encryption';
import { createFetchFn } from '@stacks/network';
import { FileContentLoader } from './fileContentLoader';
import { connectToGaiaHub, deleteFromGaiaHub, getBlockstackErrorFromResponse, getBucketUrl, getFullReadUrl, uploadToGaiaHub, } from './hub';
...
export class Storage {
    ...
    async getFileContents(path, app, username, zoneFileLookupURL, forceText, fetchFn = createFetchFn()) {
        ...
        if (!opt.encrypt && opt.sign) {
            ...
            uploadFn = async (hubConfig) => {
                const writeResponse = (await Promise.all([
                    ...
                    uploadToGaiaHub(`${path}${SIGNATURE_FILE_SUFFIX}`, signatureContent, hubConfig, 'application/json'),
                ]))[0];
            ...
            uploadFn = async (hubConfig) => {
                const writeResponse = await uploadToGaiaHub(path, contentForUpload, hubConfig, contentType, newFile, etag, opt.dangerouslyIgnoreEtag);
    ...
    async listFilesLoop(hubConfig, page, callCount, fileCount, callback, fetchFn = createFetchFn()) {
        if (callCount > 65536) {
    ...
    async setLocalGaiaHubConnection() {
        ...
        const gaiaConfig = await connectToGaiaHub(userData.hubUrl, userData.appPrivateKey, userData.gaiaAssociationToken);
...
export function getUserAppFileUrl(options) {
