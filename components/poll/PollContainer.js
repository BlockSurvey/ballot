...
export default function PollContainer(props) {
    ...
    const getDustVotingResultsForPoll = async (pollObject) => {
        ...
        try {
            ...
            if (userSession.isUserSignedIn() && currentUserAddress && allDustVoters[currentUserAddress]) {
                ...
                if (onParticipationChange) onParticipationChange(pollId, 'done');
            }
    ...
    // Embedded mode (e.g. grouped polls wizard): render only the poll content,
    // without the page-level <Head> meta tags or the dashboard navigation bar.
    if (embedded) {
        return pollContent;
    }

    // Standalone page: render exactly as the original poll page did.
    return (
        <>
            <Head>
                <title>{title}</title>
