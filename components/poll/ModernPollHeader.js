...
export default function ModernPollHeader({ pollObject, publicUrl, txStatus }) {
    ...
    useEffect(() => {
        if (descriptionRef.current && pollObject?.description) {
            const lineHeight = parseInt(window.getComputedStyle(descriptionRef.current).lineHeight);
            const height = descriptionRef.current.scrollHeight;
            const lines = Math.round(height / lineHeight);

            // Show toggle if content exceeds 5 lines
            setShouldShowToggle(lines > 5);
        }
        ...
        <>
            <div className={`${styles.poll_header} ${styles.fade_in}`}>
                <div className={styles.card_content}>
                        ...
                        <div className={styles.poll_meta_left}>
                            ...
                            <div className={styles.creator_section}>
                                <div className={styles.creator_avatar}>
                                    {pollObject?.userStxAddress?.substring(0, 2).toUpperCase()}
                                </div>
                                <div className={styles.creator_details}>
                                    ...
                                    <div className={styles.creator_meta_line}>
                                        {(pollObject?.startAtDateUTC || pollObject?.startAtDate) && (
                                            <span className={styles.poll_date}>
                    ...
                    {/* Poll Description */}
                    {pollObject?.description && (
                        <div className={styles.poll_description_wrapper}>
                            <div
                                ref={descriptionRef}
                                className={`${styles.poll_description_content} ${!isDescriptionExpanded && shouldShowToggle ? styles.poll_description_collapsed : ''
                                    }`}
                            >
                                <RichTextDisplay
