...
function ModernActionDropdown({ poll, onEditDescription, onArchive, onEditPoll, onDeleteDraft, isTableView = false }) {
                ...
                {poll?.status === "draft" ? (
                    <>
                        ...
                        <button
                            ...
                            onClick={(e) => handleItemClick('deleteDraft', e)}
                            role="menuitem"
                        >
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
...
export default function DashboardAllPollsComponent() {
    ...
    function handleDeleteDraft(poll, event) {
        event.preventDefault();
        event.stopPropagation();
    ...
    function handleDraftDeleted(pollId) {
        if (allPolls?.ref && allPolls.ref[pollId]) {
            const updatedRef = { ...allPolls.ref };
            delete updatedRef[pollId];
            const updatedList = Array.isArray(allPolls.list)
                ? allPolls.list.filter((id) => id !== pollId)
                : allPolls.list;
