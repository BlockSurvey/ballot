...
// ─── Poll Configuration ───────────────────────────────────────────────────────
const POLL_TITLE = 'SIP-044 (Clarity 6, Staking & PoX Post-Conditions, Cost-Voting Removal)'

const CONTRACT_ADDRESS = 'SP2NK4JSD4FC36P88E217XMKRCK74F1FH69MEASFA'
const CONTRACT_NAME = 'ballot-sip-044-clarity--qtjUC'
const CONTRACT_ID = `${CONTRACT_ADDRESS}.${CONTRACT_NAME}`

const SNAPSHOT_BLOCK = 8518714
const START_BLOCK = 957455
const END_BLOCK = 958925

// Options
const YES_OPTION_ID = '3b3e851c-b13b-4aa7-8e77-2be7c5f99ba1'
const NO_OPTION_ID = 'd5a957f5-3a64-4340-9d65-ec5971dcde4b'

// STX Dust Voting Addresses
const STX_YES_ADDR = 'SP000000000F5JQ6BAK9582TC1M6HRXABVZ'
const STX_NO_ADDR = 'SP00000000001Q6YBAK9582TC1M6KAW7AQP'

// BTC Dust Voting Addresses
const BTC_YES_ADDR = '11111111114Pz8NZSx6YHoTaWZkWwMc'
const BTC_NO_ADDR = '11111111111hbmNftsTSgeVBzDS7tk'

// PoX Cycles for BTC voting
const POX_CYCLES = [138, 139]
...
const getBtcDustVotes = async () => {
  ...
  // No BTC votes → skip the (expensive) PoX signer/stacker enumeration entirely
  if (btcYesEvents.length === 0 && btcNoEvents.length === 0) {
    console.log('  No BTC dust transactions found — skipping PoX map build.')
    return []
  }
...
const main = async () => {
  ...
  const prefix = `sip044-audit-${timestamp}`
